...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
		...
		while(!openList.isEmpty()) {
			...
			if (currentJNode.equals(endJNode)) {
				...
				searchedNodesNum++;
				Vector<JigsawNode> adjacentNode = findFollowJNodes(currentJNode);
				for (JigsawNode node:adjacentNode)
	...
	private void estimateValue(JigsawNode jNode) {
		...
		int s2 = 0, s3 = 0; //sum of distance, cuowei
		int dimension = JigsawNode.getDimension();

		for(int i =1 ; i < dimension*dimension; i++){
			int t = jNode.getNodesState()[i];
			if(t+1!=jNode.getNodesState()[i+1])
				s1++;
		}
		// for (int i = 1; i < dimension*(dimension-1); i++) 
		// 	if (jNode.getNodesState()[i]+dimension != jNode.getNodesState()[i+dimension])
		// 		s1++;
		for (int i = 1; i <= dimension*dimension; i++) {
			int t = jNode.getNodesState()[i];
			if (t != i && t != 0) {
				s2 += Math.abs((t-1)/dimension - (i-1)/dimension) + 
				     Math.abs((t-1)%dimension - (i-1)%dimension);
			}
			if (t != i && t != 0)
				s3++;
		}
		...
		jNode.setEstimatedValue(s1*5 + s2*10 + s3*2 +dimension);
	}
